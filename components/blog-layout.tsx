...
export async function BlogLayout({ 
  ...
  const publishedDateStr = article?.createdAt ? new Date(article.createdAt).toLocaleDateString() : "";
  const companyName = article.company ?? "Fiduciaire Premier Luxembourg S.A.";
  ...
  const otherPosts = relatedArticles?.data?.slice(0, 5);

  return (
    <>
      <div>
        <div className='max-w-[1400px] mx-auto w-full px-5 md:px-10 lg:px-20 relative z-10 pb-[60px] pt-16'>
        ...
        <div className="flex flex-col md:flex-row max-w-[1400px] mx-auto w-full px-5 md:px-10 lg:px-20 mb-16 gap-14 relative">
          
          <img className="absolute top-0 right-0 z-0" src="/svg-logo-blog.svg" alt="" width={339} height={389} />
          ...
          <div className="w-full md:w-1/3 flex flex-col mt-0 relative z-10">

            <div className="mb-10">
              <p className="text-base mb-4">{(translations as any)[locale]?.other_articles_label || (translations as any)[i18n.defaultLocale]?.other_articles_label || "Other articles"}</p>
              <ul className="flex flex-col gap-6">
                {otherPosts?.map((post: any) => (
                  <li key={post.id} className="flex gap-4 items-center">
                    <Link href={`${locale === i18n.defaultLocale ? "" : `/${locale}`}/blog/${post.slug}`} className="flex gap-4 items-center group">
                      <img
                        src={post.image?.url ? strapiImage(post.image.url) : "/thumbnail04.jpg"}
                        alt={post.title}
                        width={70}
                        height={70}
                      ...
                      />
                      <div>
                        <div className="font-medium text-[16px] leading-5 text-[#2F324A] group-hover:underline line-clamp-2">
                          {post.title}
                        </div>
            ...
            <div className='mt-0'>
                <p className='text-left'>{(translations as any)[locale]?.share_label || (translations as any)[i18n.defaultLocale]?.share_label || "Share this post"} </p>
